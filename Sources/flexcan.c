...
void FlexCANInit(void )
{
	...
	//FlexCANSetBaudrate(50, 40000);
	
#if 0
	MCF_FlexCAN_CANCTRL = MCF_FlexCAN_CANCTRL_PRESDIV(7)|
					   MCF_FlexCAN_CANCTRL_RJW(1)|
					   MCF_FlexCAN_CANCTRL_PSEG1(7)|
					   MCF_FlexCAN_CANCTRL_PSEG2(2)|
					   MCF_FlexCAN_CANCTRL_ERRMSK|
					   MCF_FlexCAN_CANCTRL_BOFFMSK|
					   MCF_FlexCAN_CANCTRL_PROPSEG(7);
	...
	MCF_FlexCAN_CANCTRL = MCF_FlexCAN_CANCTRL_PRESDIV(9)|
					   ...
					   MCF_FlexCAN_CANCTRL_PSEG2(4)|
					   MCF_FlexCAN_CANCTRL_ERRMSK|
					   MCF_FlexCAN_CANCTRL_BOFFMSK|
					   MCF_FlexCAN_CANCTRL_PROPSEG(4);
	*/
	
	MCF_FlexCAN_CANCTRL = MCF_FlexCAN_CANCTRL_PRESDIV(7)|
						   MCF_FlexCAN_CANCTRL_RJW(3)|
						   MCF_FlexCAN_CANCTRL_PSEG1(7)|
						   MCF_FlexCAN_CANCTRL_PSEG2(7)|
						   MCF_FlexCAN_CANCTRL_ERRMSK|
						   MCF_FlexCAN_CANCTRL_BOFFMSK|
    ...
    MCF_FlexCAN_CANCTRL = MCF_FlexCAN_CANCTRL_PRESDIV(23)|
				   MCF_FlexCAN_CANCTRL_RJW(3)|
				   MCF_FlexCAN_CANCTRL_PSEG1(4)|
				   MCF_FlexCAN_CANCTRL_PSEG2(5)|
				   MCF_FlexCAN_CANCTRL_PROPSEG(0);
	/*
	MCF_FlexCAN_CANCTRL = MCF_FlexCAN_CANCTRL_PRESDIV(15)|
					MCF_FlexCAN_CANCTRL_RJW(1)|
					MCF_FlexCAN_CANCTRL_PSEG1(7)|
					MCF_FlexCAN_CANCTRL_PSEG2(2)|
					MCF_FlexCAN_CANCTRL_ERRMSK|
...
*/
	...
	MCF_FlexCAN_RXGMASK = 0x0;
	MCF_FlexCAN_IMASK = 0x1;
	FlexCANEnableMBInterrupt(0, 1);
...
int8 FlexCANSendDataPoll(uint8 *pData, uint8 u8Size, uint32 u32ID, uint8 u8MB)
{
    ...
    printf("1.STAT:%x\tCNT:%x\n", MCF_FlexCAN_ERRSTAT, MCF_FlexCAN_ERRCNT);
    ...
    printf("2.STAT:%x\tCNT:%x\n", MCF_FlexCAN_ERRSTAT, MCF_FlexCAN_ERRCNT);
    /* transmit the frame */
    (*BuffPtr).u16CtrlStat = MB_CTRLSTAT_CODE(0xC)|temp;

    while (!(MCF_FlexCAN_IFLAG & (1<<u8MB))){
        if (u32Rescue++ == RESCUE_VALUE){
        	printf("3.STAT:%x\tCNT:%x\n", MCF_FlexCAN_ERRSTAT, MCF_FlexCAN_ERRCNT);
            return 1;
        }
    }
    MCF_FlexCAN_IFLAG = (1<<u8MB);

	printf("OK\tSTAT:%x\tCNT:%x\tRescue=%d\n", MCF_FlexCAN_ERRSTAT, MCF_FlexCAN_ERRCNT, u32Rescue);
#if 0
	if (MCF_FlexCAN_ERRSTAT & (1 << 5)){
		MCF_FlexCAN_CANMCR = MCF_FlexCAN_CANMCR_SOFTRST;
		while ((MCF_FlexCAN_CANMCR & MCF_FlexCAN_CANMCR_SOFTRST) != 0x00);
	}
#endif
...
uint8 FlexCANReceiveDataPoll(uint8 u8MB, uint8 *aData)
{
    ...
    if(u8Temp & (1 << 5)){
    	// remote frame
    	stdid = ((*BuffPtr).u32ID.u32StdIDField.u16StdID) >> 2;
    	extid = ((*BuffPtr).u32ID.u32ExtID) << 14;
    }else
    	stdid = ((*BuffPtr).u32ID.u32StdIDField.u16StdID) >> 2;
    ...
    printf("Recv ID=%x\tlen=%d\tMSG=", stdid, u8Len);
    for (i=0; i < u8Len; i++){
        printf("%x", pPointer[i]);
    }
    printf("\n");
